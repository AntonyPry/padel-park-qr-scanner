...
const ONBOARDING_ROUTE_START_COMMANDS = {
  ...
  '/admin/shift-settings': 'Открой раздел «Настройки смены»',
  '/admin/shift/cash': 'Открой раздел «Смена» → «Касса»',
  '/admin/shift/motivation': 'Открой раздел «Смена» → «Мотивация»',
  '/admin/shift/reports': 'Открой раздел «Смена» → «Отчеты»',
      ...
      },
      {
        ...
        tasks: [
          ...
          },
          {
            ...
            description:
              'Внести сумму, описание, фото чека и проверить, что расход уменьшил ожидаемый остаток.',
            route: '/admin/shift/cash',
            kind: 'action',
      ...
      },
      {
        ...
        tasks: [
          ...
          },
          {
            ...
            description:
              'Открыть шаблоны отчетов, создать шаблон, отредактировать активный, архивировать и восстановить его через модалку.',
            route: '/admin/shift-settings',
            kind: 'review',
            skills: ['Отчеты смены', 'Настройки'],
            badge: 'Шаблон понятен',
            estimatedMinutes: 7,
            rewardXp: 35,
            checkpoint: {
              event: 'report.viewed',
              conditions: { report: 'shift_report_templates', taskKey: 'manager.shift-report-templates.manage' },
            },
            trainingMode: { recommended: false },
          },
        ],
      ...
      },
      {
        ...
        tasks: [
          ...
          },
          {
            ...
            description:
              'Открыть шаблоны отчетов, проверить создание, редактирование, архив, read-only состояние и восстановление.',
            route: '/admin/shift-settings',
            kind: 'review',
            skills: ['Отчеты смены', 'Контроль'],
            badge: 'Шаблоны под контролем',
            estimatedMinutes: 7,
            rewardXp: 35,
            checkpoint: {
              event: 'report.viewed',
              conditions: { report: 'shift_report_templates', taskKey: 'owner.shift-report-templates.manage' },
            },
            trainingMode: { recommended: false },
          },
          {
    ...
    missions: [
      {
        ...
        tasks: [
          ...
          },
          {
            ...
            description:
              'Открыть финансы, найти расход, созданный из кассы смены, и проверить сумму, дату, комментарий и связанную историю.',
            route: '/admin/finances',
            kind: 'review',
            skills: ['Касса', 'P&L'],
            badge: 'Расход сверён',
...
const SHIFT_REPORT_TEMPLATES_UPDATED_AT = '2026-07-17T12:00:00.000+03:00';
...
function makeShiftReportTemplateScreenshot(fileName, alt, caption) {
  return {
    src: `/onboarding/shift-report-templates/${fileName}`,
    alt,
    caption,
  };
}

const SHIFT_REPORT_TEMPLATE_SCREENSHOTS = [
  makeShiftReportTemplateScreenshot(
    'templates-list.png',
    'Список шаблонов отчетов смены',
    'В списке видны карточки шаблонов: название, статус, расписание, времена отчетов и описание. Пункты отчета открываются внутри модалки шаблона.',
  ),
  makeShiftReportTemplateScreenshot(
    'create-modal.png',
    'Модалка создания шаблона отчета смены',
    'Создание выполняется в модалке: название, опоздание, описание, расписание и пункты отчета.',
  ),
  makeShiftReportTemplateScreenshot(
    'active-edit-modal.png',
    'Модалка редактирования активного шаблона отчета смены',
    'Активный шаблон редактируется в модалке; сохранение и архивирование находятся внизу модалки.',
  ),
  makeShiftReportTemplateScreenshot(
    'archived-readonly-modal.png',
    'Архивный шаблон отчета смены в режиме чтения',
    'У архивного шаблона поля недоступны для редактирования, сохранение выключено, восстановление доступно в модалке.',
  ),
];
...
const roleInstructionLessons = {
    ...
    blocks: [
      {
        ...
        type: 'step',
        text:
          'Открой «Смена» и выбери вкладку «Касса». Если смена еще не начата, сначала начни смену в рабочей панели раздела, затем вернись на вкладку «Касса».',
      },
      {
        screenshotIndex: 2,
    ...
    screenshots: [
      {
        ...
        caption:
          'Рабочее пространство «Смена» открыто на вкладке «Касса»: сверху видны подразделы «Мотивация», «Отчеты» и «Касса».',
      },
      {
        src: '/onboarding/shift-cash/full-cash-metrics.png',
        alt: 'Касса активной смены с верхними метриками',
        caption:
          'Вкладка «Касса» показывает состояние текущей смены перед фиксацией стартового остатка.',
      },
      {
    ...
    blocks: [
      {
        ...
        type: 'step',
        text:
          'Открой «Смена» → «Касса» и перейди к форме «Добавить расход из кассы». Если начальный остаток еще не зафиксирован, сначала заполни его.',
      },
      {
        screenshotIndex: 2,
        title: 'Что заполнить: сумма, описание и фото',
        type: 'step',
        text:
          'Укажи сумму расхода, напиши понятное описание и добавь фото чека. После проверки нажми «Добавить расход».',
        items: [
      ...
      },
      {
        ...
        type: 'step',
        text:
          'После сохранения расход должен появиться в списке расходов смены, у строки должны быть сумма, описание, автор, время и прикрепленное фото чека. Сумма расходов должна увеличиться, а ожидаемый остаток наличности уменьшиться.',
      },
    ],
    screenshots: [
      {
        ...
        caption:
          'Вкладка «Касса» в разделе «Смена» перед добавлением расхода: метрики, стартовый остаток и форма расхода.',
      },
      {
        src: '/onboarding/shift-cash/full-cash-metrics.png',
        alt: 'Касса активной смены перед добавлением расхода',
      ...
      },
      {
        ...
        caption:
          'Форма расхода содержит сумму, описание, загрузку фото чека и кнопку «Добавить расход».',
      },
      {
        src: '/onboarding/shift-cash/expense-result-attached.png',
        alt: 'Сохраненный кассовый расход с прикрепленным чеком',
        caption:
          'В карточке расхода видны сумма, описание, автор, время и превью чека.',
      },
    ],
  }),
    ...
    blocks: [
      {
        ...
        type: 'step',
        text:
          'Открой «Смена» → «Касса». Проверь, что работаешь с нужной активной сменой и видишь верхние метрики: на начало, наличная выручка, расходы, ожидаемый остаток и факт.',
      },
      {
        ...
        items: [
          ...
          'Расходы из кассы должны иметь понятное описание и фото чека, если чек есть.',
        ],
      },
      {
        screenshotIndex: 4,
    ...
    screenshots: [
      {
        ...
        caption:
          'Вкладка «Касса» показывает метрики смены, стартовый остаток и зону расходов.',
      },
      {
        src: '/onboarding/shift-cash/full-cash-metrics.png',
        alt: 'Полные метрики кассы активной смены',
      ...
      },
      {
        ...
        caption:
          'Сохраненный расход показывает сумму, описание, автора, время и прикрепленный чек.',
      },
      {
    ...
    blocks: [
      {
        ...
        type: 'step',
        text:
          'Открой «Смена» → «Касса». Владелец может проходить обучение за роли, но права владельца при этом сохраняются.',
      },
      {
        screenshotIndices: [2, 3],
        ...
        type: 'step',
        text:
          'Проверь стартовый остаток, наличную выручку, расходы, ожидаемый остаток и факт. По расходам смотри описание, фото чека и статус: активный или отмененный.',
        items: [
      ...
      },
      {
        ...
        type: 'step',
        text:
          'Результат проверки нормальный, если закрытая касса показывает факт, расхождение и комментарий, а кассовые расходы можно найти в финансовом отчете по дате смены, сумме и связанной строке истории.',
      },
    ],
    screenshots: [
      {
        ...
        caption:
          'Вкладка «Касса» показывает метрики смены, стартовый остаток и расходы смены.',
      },
      {
        src: '/onboarding/shift-cash/full-cash-metrics.png',
        alt: 'Кассовый блок активной смены для владельца',
      ...
      {
        src: '/onboarding/shift-cash/expense-result-attached.png',
        alt: 'Расход смены с описанием, суммой и чеком',
        caption:
          'Карточка расхода показывает сумму, описание, автора, время и фото чека.',
      },
      {
        src: '/onboarding/shift-cash/close-dialog-variance-comment.png',
  ...
  'accountant.shift-cash.finance-review': makeTextLesson({
    ...
    blocks: [
      ...
      {
        screenshotIndex: 2,
        title: 'Что сверить: сумма, дата и комментарий',
        type: 'step',
        text:
          'Найди строку расхода в P&L и связанную запись в финансовой истории. Сверь, что сумма, дата смены, комментарий и причина операции совпадают с кассовым расходом.',
        items: [
          ...
          'Если сумма или комментарий ошибочные, исправление делает роль с правом управления финансами или кассой по процессу клуба.',
        ],
      },
      {
  ...
  'manager.shift-report-templates.manage': makeTextLesson({
    title: 'Как настроить шаблон отчета смены',
    summary:
      'Шаблон задает, какие отчеты создаются для конкретной смены. Сотрудник берется из самой смены, а все активные шаблоны применяются по расписанию.',
    updatedAt: SHIFT_REPORT_TEMPLATES_UPDATED_AT,
    screenshots: SHIFT_REPORT_TEMPLATE_SCREENSHOTS,
    blocks: [
      {
        title: 'Открой «Настройки смены» → «Шаблоны отчетов»',
        type: 'step',
        text:
          'Открой раздел «Настройки смены» и перейди к шаблонам отчетов. В списке должны быть карточки с названием, статусом, временем отчета и описанием.',
        items: [
          'На карточке больше нет устаревших полей области действия шаблона.',
          'Пункты отчета проверяй после открытия карточки в модалке.',
          'Карточка не содержит отдельный набор action-кнопок: клик по карточке, Enter или Space открывает модалку.',
        ],
        screenshotIndex: 0,
      },
      {
        title: 'Создай шаблон в модалке',
        type: 'step',
        text:
          'Нажми «Создать шаблон», заполни название, допустимое опоздание, описание, добавь время отчета и хотя бы один пункт. После проверки нажми «Создать».',
        items: [
          'Отчет создается для конкретной смены; сотрудник определяется самой сменой.',
          'Все активные шаблоны применяются по расписанию, поэтому времена и пункты должны быть понятны администратору смены.',
        ],
        screenshotIndex: 1,
      },
      {
        title: 'Открой активную карточку и сохрани изменение',
        type: 'step',
        text:
          'Открой существующую активную карточку. В модалке измени только нужное поле: название, описание, время, опоздание или пункт отчета. После проверки нажми «Сохранить».',
        items: [
          'Не ищи старые поля области действия шаблона: они больше не настраиваются.',
          'Если шаблон больше не нужен, архивируй его кнопкой «Архивировать шаблон» внутри этой же модалки.',
        ],
        screenshotIndex: 2,
      },
      {
        title: 'Проверь архив и восстановление',
        type: 'step',
        text:
          'Открой архивную карточку. Поля должны быть read-only, кнопка сохранения выключена, а восстановление доступно в модалке. Если шаблон снова нужен, нажми «Восстановить шаблон».',
        items: [
          'После восстановления вернись к списку и проверь, что шаблон снова активен.',
          'После архивации проверь, что карточка получила статус «В архиве» и не используется как активный шаблон.',
        ],
        screenshotIndex: 3,
      },
      {
        title: 'Проверь результат в списке',
        type: 'paragraph',
        text:
          'Результат корректен, если карточка показывает осмысленные данные шаблона: название, статус, расписание, время отчета и описание. Пункты отчета проверяются внутри модалки, а в тексте и на карточке не должно быть устаревших полей области действия шаблона.',
      },
    ],
  }),
  ...
  'owner.shift-report-templates.manage': makeTextLesson({
    title: 'Как проверить шаблоны отчетов смены',
    summary:
      'Владелец проверяет, что шаблоны отчетов создают понятные задания для смены: сотрудник определяется сменой, а активные шаблоны применяются по расписанию.',
    updatedAt: SHIFT_REPORT_TEMPLATES_UPDATED_AT,
    screenshots: SHIFT_REPORT_TEMPLATE_SCREENSHOTS,
    blocks: [
      {
        ...
        type: 'step',
        text:
          'Открой раздел «Настройки смены» и перейди к шаблонам отчетов. Начинай проверку со списка карточек: название, статус, расписание, время отчета и описание.',
        items: [
          'Карточка открывается кликом, Enter или Space.',
          'Пункты отчета находятся внутри модалки открытого шаблона.',
          'Устаревшие поля области действия шаблона не должны отображаться ни в карточке, ни в модалке.',
        ],
        screenshotIndex: 0,
      },
      {
        title: 'Проверь создание',
        type: 'step',
        text:
          'Нажми «Создать шаблон». В модалке задай название, опоздание, описание, время отчета и пункты. После проверки нажми «Создать».',
        items: [
          'Шаблон не выбирает отдельную область действия: отчет появится у смены, а сотрудник берется из данных этой смены.',
          'Если активных шаблонов несколько, каждый применится по своему расписанию.',
        ],
        screenshotIndex: 1,
      },
      {
        title: 'Проверь редактирование и архив',
        type: 'step',
        text:
          'Открой активный шаблон. Измени только нужное поле и нажми «Сохранить». Если шаблон надо вывести из работы, нажми «Архивировать шаблон» в этой же модалке.',
        items: [
          'Сохранение должно менять карточку в списке.',
          'Архивирование должно сменить статус карточки на «В архиве».',
        ],
        screenshotIndex: 2,
      },
      {
        title: 'Проверь read-only архив и восстановление',
        type: 'step',
        text:
          'Открой архивную карточку. Поля должны быть disabled, кнопка «Сохранить» недоступна, а «Восстановить шаблон» доступна. После восстановления проверь, что карточка снова активна.',
        items: [
          'Архивный шаблон можно прочитать, но нельзя случайно изменить.',
          'Восстановление выполняется из модалки, а не отдельной кнопкой на карточке.',
        ],
        screenshotIndex: 3,
      },
      {
        ...
        type: 'paragraph',
        text:
          'Раздел настроен корректно, если владелец видит только актуальную модель шаблонов: список карточек, модалку создания, модалку редактирования, архивный read-only режим и восстановление без устаревших полей области действия шаблона.',
      },
    ],
  }),
...
const CRM_KNOWLEDGE_SECTIONS = [
  ...
  {
    slug: 'motivation',
    title: 'Мотивация и шаблоны отчетов',
    description:
      'Разобраться, как правила мотивации связаны с начислениями, а шаблоны отчетов - с контролем смены.',
    route: '/admin/shift-settings',
    skills: ['Мотивация', 'Шаблоны отчетов'],
    badge: 'Логика смены',
    estimatedMinutes: 7,
    updatedAt: SHIFT_REPORT_TEMPLATES_UPDATED_AT,
    summary:
      'Настройки смены содержат правила мотивации и шаблоны отчетов, которые влияют на работу смены.',
    cards: [
      {
        title: 'Что показывает раздел настроек',
        text:
          'В «Настройках смены» есть правила мотивации и шаблоны отчетов. Правила мотивации задают начисления, а шаблоны отчетов задают, какие отчеты создаются для смен по расписанию.',
      },
      {
      ...
      {
        title: 'Как работает шаблон отчета',
        text:
          'Шаблон отчета не выбирает отдельную область действия. Отчет создается для конкретной смены, сотрудник определяется самой сменой, а все активные шаблоны применяются по своему расписанию.',
      },
      {
        title: 'Как не создать конфликт',
        text:
          'Перед изменением правила мотивации нужно понимать, на какие категории и периоды оно влияет. Перед изменением шаблона отчета проверь, какие пункты увидит администратор смены и когда отчет должен появиться.',
      },
    ],
    managerLens:
      'Менеджер открывает «Настройки смены», чтобы проверить правила мотивации и шаблоны отчетов: какие условия участвуют в начислениях и какие отчеты создаются для смен.',
    ownerLens:
      'В «Настройках смены» владелец в CRM проверяет базовые ставки, правила переработки, бонусные правила, категории, пороги, периоды действия и шаблоны отчетов смены. Экран показывает, какие правила участвуют в начислениях и какие отчеты создаются по расписанию.',
  },
  {
    slug: 'utilization',
  ...
  motivation: [
    {
      title: 'Карта экрана для первого входа',
      text:
        'Сначала раздели экран на две части: правила мотивации отвечают за начисления, шаблоны отчетов - за контроль смены. Правило мотивации надо читать вместе с начислениями и каталогом, а шаблон отчета - вместе с расписанием смен.',
    },
    {
    ...
    {
      title: 'Связь с отчетами смены',
      text:
        'Шаблон отчета создает отчеты для конкретных смен. В шаблоне настраиваются название, расписание, опоздание, описание и пункты отчета; устаревшие поля области действия не являются свойствами шаблона.',
    },
    {
      title: 'Типовые ошибки и исключения',
      text:
