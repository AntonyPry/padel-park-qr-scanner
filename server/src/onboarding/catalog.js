            ...
            quiz: {
              ...
              questions: [
                ...
                },
                {
                  ...
                  explanation:
                    'Режим обучения помечает созданного клиента как учебного, чтобы его можно было удалить и не учитывать в боевых отчетах.',
                  options: [
                    {
      ...
      },
      {
        ...
        tasks: [
          ...
          },
          {
            ...
            title: 'Понять списание абонемента клиента',
            description:
              'Открыть карточку клиента и понять, где видны активные абонементы, остаток занятий, срок действия и история списаний.',
            route: '/admin/clients',
            kind: 'review',
            skills: ['Абонементы', 'Клиенты'],
            badge: 'Абонемент прочитан',
            estimatedMinutes: 6,
            rewardXp: 35,
            checkpoint: {
...
function makeTextLesson({ blocks, summary, title }) {
  return {
    title,
    summary,
...
const roleInstructionLessons = {
    ...
    blocks: [
      {
        ...
        text:
          'Для администратора единый экран предоплат показывает доступные операционные блоки: абонементы и сертификаты. Он нужен как навигация к разрешенным деталям, а не как финансовая настройка.',
      },
      {
        title: 'Как читать абонементы',
        text:
          'Абонемент показывает клиента, тип, остаток занятий, использовано, срок действия и статус. Если нужно списать тренировку, переходи в карточку клиента и проверяй историю абонемента там.',
      },
      {
        title: 'Как читать сертификаты',
        text:
          'Сертификат показывает код, клиента, тип, остаток денег или услуг, срок действия и статус. Если клиент пришел с кодом, открывай сертификат и сверяй остаток перед списанием.',
      },
      {
        title: 'Граница роли администратора',
        text:
          'Администратор работает только с доступными ему операционными деталями. Если вопрос требует закрытого финансового блока или настройки, передай его управляющему, владельцу или бухгалтеру по роли.',
      },
    ],
  }),
  'admin.subscription.redemption-review': makeTextLesson({
    title: 'Как понять списание абонемента клиента',
    summary:
      'Абонемент в карточке клиента отвечает на два рабочих вопроса: сколько занятий осталось и можно ли списать занятие сейчас.',
    blocks: [
      {
        title: 'Где смотреть абонемент',
        text:
          'Открой карточку клиента и найди блок абонементов. Активные абонементы показываются отдельно от истории, чтобы сотрудник на смене быстро видел доступный остаток.',
      },
      {
        title: 'Какие поля важны',
        text:
          'Смотри тип абонемента, формат тренировки, общий лимит занятий, использовано, остаток, срок действия, статус и историю списаний. Если абонемент безлимитный, остаток не уменьшается как у пакетного тарифа.',
      },
      {
        title: 'Когда списание недоступно',
        text:
          'Нельзя списывать истекший, использованный, отмененный или неподходящий абонемент. Если данные клиента или тип тренировки не совпадают с фактом занятия, сначала уточни ситуацию, а не списывай запись наугад.',
      },
      {
        title: 'Что остается в истории',
        text:
          'Каждое списание хранит дату, услугу, сотрудника и комментарий. Ошибочное списание отменяется отдельным действием, чтобы история показывала и исходное действие, и исправление.',
      },
    ],
  }),
  'admin.certificate.redemption-review': makeTextLesson({
    ...
    blocks: [
      {
        title: 'Как найти сертификат',
        text:
          'В разделе сертификатов используй поиск по коду, клиенту или статусу. Код является главным ориентиром: по нему можно быстро проверить, активен ли сертификат и какой остаток доступен.',
      },
      {
        title: 'Денежный и сервисный сертификат',
        text:
          'Денежный сертификат списывается суммой. Сервисный сертификат списывается количеством услуг или пакетом. Поэтому перед списанием важно понять тип сертификата, а не ориентироваться только на название.',
      },
      {
        title: 'Как читать остаток',
        text:
          'CRM показывает исходный номинал, уже использованную часть, текущий остаток и срок действия. Если остаток нулевой или срок истек, сертификат нельзя считать доступным для новой услуги.',
      },
      {
        title: 'История и исправления',
        text:
          'Списание сертификата оставляет историю с датой, сотрудником и комментарием. Ошибочное списание отменяется отдельной записью, чтобы не потерять след исправления.',
      },
    ],
  }),
  'admin.booking.training-plan-link': makeTextLesson({
    ...
    blocks: [
      {
        title: 'Какая бронь подходит',
        text:
          'План тренировки доступен только для тренировочных типов брони: персональная тренировка или групповая тренировка. Обычная игровая бронь не должна создавать тренировочный план, потому что у нее нет методического сценария занятия.',
      },
      {
        title: 'Что CRM берет из брони',
        text:
          'Из брони используются дата, время, корт, тип занятия, клиент или участники группы и ответственный сотрудник. Если ответственный сотрудник связан с активным аккаунтом тренера, план назначается этому тренеру.',
      },
      {
        title: 'Как появляется план',
        text:
          'Когда пользователь открывает действие плана у тренировочной брони, CRM проверяет, есть ли план уже. Если плана нет, система строит рекомендацию по карте навыков участников и сохраняет запланированный план с упражнениями из утвержденной методической базы.',
      },
      {
        title: 'Что проверять администратору',
        text:
          'Перед созданием плана важно проверить тип брони, участника или состав группы и ответственного тренера. Если эти данные ошибочны, план попадет не тому тренеру или будет собран по неверным игрокам.',
      },
    ],
  }),
    ...
    blocks: [
      {
        ...
        text:
          'Смотри корпоративные остатки: компанию, текущий баланс, признаки низкого остатка и переход к корпоративной карточке. Остальные блоки сводки могут быть скрыты для роли бухгалтера.',
      },
      {
        title: 'Связь с финансовым отчетом',
        text:
          'Корпоративные пополнения идут через ручные финансовые операции. Поэтому бухгалтер сверяет не только баланс компании, но и связанную запись в финансах, чтобы отчет прибыли не расходился с корпоративным остатком.',
      },
      {
        title: 'Какие суммы не путать',
        text:
          'Корпоративный баланс уменьшается списаниями услуг и увеличивается пополнениями. Финансовая операция показывает приход денег, а баланс показывает, сколько компания еще может использовать.',
      },
      {
        title: 'Когда нужна сверка',
        text:
          'Если корпоративный баланс не совпадает с оплатами, проверь связанные финансовые операции, активные списания, отмененные операции и экспорт детализации за период.',
      },
    ],
  }),
  'accountant.corporate.deposit-review': makeTextLesson({
    ...
    blocks: [
      {
        title: 'Два слоя одной операции',
        text:
          'Запись баланса нужна для детализации компании. Финансовая операция нужна для отчета прибыли. Если пополнение не связано с финансами, баланс может быть верным, но финансовый отчет потеряет доход.',
      },
      {
        title: 'Создать или связать',
        text:
          'CRM может создать ручной финансовый доход из пополнения или связать пополнение с уже существующей финансовой операцией. В обоих случаях нужно проверить сумму, дату, категорию и комментарий.',
      },
      {
        title: 'Учебная безопасность',
        text:
          'В режиме обучения корпоративный клиент, пополнение и связанная финансовая операция должны иметь учебный признак. Иначе учебное пополнение попадет в боевой финансовый отчет.',
      },
      {
        title: 'Отмена пополнения',
        text:
          'Отмена должна оставить историю и синхронно убрать влияние на баланс. Если финансовая операция была создана из пополнения, ее тоже нужно учитывать в логике отмены.',
      },
    ],
  }),
  'accountant.corporate.export-review': makeTextLesson({
    ...
    blocks: [
      {
        title: 'Что попадает в выгрузку',
        text:
          'В выгрузке должны быть дата, тип операции, услуга, участник или клиент, сумма, комментарий и остаток после операции. Так компания видит, как использовались ее деньги.',
      },
      {
        title: 'Как выбрать период',
        text:
          'Период должен совпадать с запросом компании или отчетным месяцем. Слишком широкий период усложнит сверку, слишком узкий может скрыть пополнение или отмену.',
      },
      {
        title: 'Как читать отмены',
        text:
          'Отмененная операция остается в истории как исправление. При проверке итогового остатка важно понимать, какие строки активны, а какие только объясняют прошлое исправление.',
      },
      {
  ...
  'trainer.methodology.review-base': makeTextLesson({
    ...
    blocks: [
      {
        title: 'Что видно тренеру',
        text:
          'Тренер видит активные навыки и утвержденные упражнения, а также свои черновики упражнений. Это позволяет готовиться к занятиям и предлагать новые упражнения без изменения общей базы без проверки.',
      },
      {
        title: 'Как читать упражнение',
        text:
          'У упражнения есть главный навык, дополнительные навыки, ступень, формат, диапазон уровня навыка, критерий успеха, упрощение и усложнение. Эти поля объясняют, кому упражнение подходит и как менять сложность на занятии.',
      },
      {
        title: 'Почему важен статус',
        text:
          'Черновик не участвует в рекомендациях и автоматических планах. Утвержденное упражнение участвует. Архивное остается для истории, но не должно попадать в новые рекомендации.',
      },
      {
        title: 'Граница роли',
        text:
          'Тренер не утверждает общую методику сам. Если нужен новый рабочий вариант, создай черновик с понятным критерием успеха и адаптациями, чтобы управляющий мог проверить его для общей базы.',
      },
    ],
  }),
  'trainer.client.skill-map-review': makeTextLesson({
    ...
    blocks: [
      {
        title: 'Основные поля карты',
        text:
          'У каждого навыка есть уровень от 0 до 5, дата последней отработки, последнее упражнение, последняя оценка, флаг повтора и следующая ступень упражнения. Эти поля помогают выбрать фокус следующего занятия.',
      },
      {
        title: 'Откуда берется карта',
        text:
          'CRM создает строки карты по активным навыкам методической базы. Когда тренер фиксирует структурные результаты упражнения, CRM пересчитывает состояние навыка и сохраняет историю изменений.',
      },
      {
        title: 'Что значит флаг повтора',
        text:
          'Repeat flag означает, что навык или упражнение нужно повторить. Он появляется после низкой оценки, явной отметки тренера или ситуации, где навык не готов к повышению.',
      },
      {
        title: 'Приватность тренера',
        text:
          'В тренерском кабинете карта должна читаться без телефонов, внешних идентификаторов и лишних CRM-полей клиента. Тренеру нужен тренировочный контекст, а не управление клиентской базой.',
      },
    ],
  }),
  'trainer.training-note.structured-record': makeTextLesson({
    ...
    blocks: [
      {
        title: 'Что выбрать в записи',
        text:
          'Выбери дату, уровень игрока, упражнения из утвержденной базы, оценку выполнения и при необходимости флаг повтора по упражнению или навыку. Свободная заметка остается для короткого вывода, а не для персональных данных.',
      },
      {
        title: 'Как работает оценка',
        text:
          'Оценка 1/5 или 2/5 не снижает уровень, но ставит повтор и предлагает вернуться на шаг ниже. Оценка 3/5 оставляет уровень и требует закрепления. Две подходящие оценки 4/5 или 5/5 без флага повтора могут повысить уровень навыка.',
      },
      {
        title: 'Как CRM обновляет карту',
        text:
          'После сохранения CRM пересчитывает карту по структурной истории: упражнение, ступень, диапазон уровня навыка, оценка, флаг повтора и дата занятия. История изменения остается в карте навыка.',
      },
      {
        title: 'Что проверить после сохранения',
        text:
          'Проверь, что запись появилась в дневнике, упражнения отображаются списком, карта навыков обновилась ожидаемо, а текст заметки не содержит лишних персональных данных.',
      },
    ],
  }),
  'trainer.recommendation.personal-review': makeTextLesson({
    ...
    blocks: [
      {
        title: 'Какие данные учитываются',
        text:
          'CRM смотрит карту навыков, последний уровень игры, последние тренировки, оценки, флаг повтора, давность отработки и цель занятия. Чем точнее структурная история, тем полезнее рекомендация.',
      },
      {
        title: 'Как выбираются навыки',
        text:
          'Приоритет получают слабые навыки, давно не отработанные навыки, навыки с низкой последней оценкой, флагом повтора или совпадением с целью занятия. Недавно успешно закрепленный навык получает меньший приоритет.',
      },
      {
        title: 'Как выбираются упражнения',
        text:
          'CRM ищет утвержденные упражнения по навыку, формату, ступени и диапазону уровня навыка. Она старается не повторять упражнения из последних трех тренировок, если тренер не поставил флаг повтора или нет подходящих альтернатив.',
      },
      {
        title: 'Когда нужен ручной блок',
        text:
          'Если в базе нет подходящего упражнения или все варианты нарушают правило повторов, CRM оставляет ручной блок с объяснением. Это сигнал пополнить методическую базу или вручную адаптировать занятие.',
      },
    ],
  }),
  'trainer.recommendation.group-review': makeTextLesson({
    ...
    blocks: [
      {
        title: 'Какие участники учитываются',
        text:
          'В группу можно выбрать несколько игроков. CRM смотрит карту навыков каждого участника, общие слабые места, флаги повтора, историю упражнений и цель занятия.',
      },
      {
        title: 'Как строится план группы',
        text:
          'План делится на блоки: разминка, навык большинства, дифференцированная отработка, игровая связка и мини-игра. Для каждого блока CRM пытается выбрать упражнение, которое подходит формату и уровню группы.',
      },
      {
        title: 'Что значит дифференциация',
        text:
          'Участники могут иметь разные уровни одного навыка. Поэтому рекомендация может предложить общий фокус, но дать тренеру причины для упрощения, усложнения или ручной замены упражнения.',
      },
      {
        title: 'Что проверять тренеру',
        text:
          'Перед созданием плана проверь участников, цель, выбранные упражнения и блоки без автоматической вставки. Если блок оставлен вручную, не игнорируй его: CRM показывает, почему автоматический выбор был небезопасен.',
      },
    ],
  }),
  'trainer.training-plan.lifecycle': makeTextLesson({
    ...
    blocks: [
      {
        title: 'Что значит запланированный план',
        text:
          'Planned-план содержит дату, тренера, участников, цель и запланированные упражнения. Его можно создать вручную, из персональной рекомендации, групповой рекомендации или из тренировочной брони.',
      },
      {
        title: 'Как связан план с бронью',
        text:
          'Если бронь имеет тип персональной или групповой тренировки и ответственного тренера, CRM может создать план прямо из брони. Дата, участники и ответственный берутся из бронирования.',
      },
      {
        title: 'Что можно менять до занятия',
        text:
          'Пока план запланирован, тренер может заменить упражнения, если занятие требует адаптации. Важно не дублировать одно упражнение в плане и выбирать утвержденные упражнения из базы.',
      },
      {
        title: 'Что значит завершенный план',
        text:
          'После завершения CRM создает или обновляет тренировочные записи участников, фиксирует фактические упражнения и запускает пересчет карты навыков по структурным результатам.',
      },
    ],
  }),
...
const CRM_KNOWLEDGE_DEEP_CARDS = {
  ...
  trainer: [
    ...
    },
    {
      ...
      text:
        'Тренер выбирает игрока, фиксирует дату, уровень, упражнения из методической базы, оценку и вывод. Запись остается в дневнике, а структурные результаты запускают пересчет карты навыков. Уровень меняется только при понятном тренировочном основании.',
    },
    {
      title: 'Алгоритм обновления навыков',
      text:
        'CRM смотрит оценку, ступень упражнения, диапазон уровня, флаг повтора и историю последних подходящих результатов. Низкая оценка ставит повтор. Оценка 3/5 закрепляет текущий уровень. Две подходящие оценки 4/5 или 5/5 без повтора могут повысить уровень навыка.',
    },
    {
...
const KNOWLEDGE_MISSION_GROUPS = [
  {
    ...
    sections: ['access-monitor', 'bookings', 'clients'],
  },
  {
