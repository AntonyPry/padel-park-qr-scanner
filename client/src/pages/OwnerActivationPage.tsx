    ...
    <main className="min-h-screen bg-muted/35 p-4 sm:p-8">
      ...
      <div className="mx-auto flex min-h-[calc(100vh-7rem)] max-w-5xl items-center justify-center py-8">
        <Card className="w-full max-w-lg rounded-2xl shadow-lg shadow-foreground/5">
          ...
          {!loading && activatedEmail ? <><CardHeader className="space-y-4"><div className="flex size-12 items-center justify-center rounded-xl bg-emerald-500/10 text-emerald-700 dark:text-emerald-300"><CheckCircle2 className="size-6" /></div><div><CardTitle>Аккаунт владельца активирован</CardTitle><p className="mt-2 text-sm leading-6 text-muted-foreground">Теперь войдите в Setly как <span className="font-medium text-foreground">{activatedEmail}</span>.</p></div></CardHeader><CardContent><Button className="w-full" asChild><a href="/">Перейти ко входу в Setly</a></Button></CardContent></> : null}
