...
Для design/redesign/UI prototype чатов живой просмотр обязателен: запусти проект или прототип, дай кликабельные frontend/backend URLs, роль/demo account и manual QA сценарии. Screenshots не являются deliverable по умолчанию и не блокируют preview/handoff: создавай их только по явному запросу пользователя либо когда они нужны Feature/QA для внутренней визуальной проверки, сравнения или диагностики; не выгружай внутренние screenshots в handoff без пользы. Product screenshot assets для onboarding-урока остаются отдельным обязательным контентом. Если dev server или сама браузерная проверка невозможны, это blocker/known risk.
...
- Для frontend-нововведений `lint/build` и login smoke не считаются полноценной проверкой. Делай visual QA desktop и мобильного `390px` с реалистичными длинными данными, проверкой `document.scrollWidth`, безопасных отступов от краев, console/network/page errors. Screenshots сохраняй только по явному запросу или когда они реально нужны для внутренней проверки/диагностики. Если локальная БД недоступна, сначала используй mocked API для layout QA, а после восстановления БД повтори DB-backed сценарий.
- Переиспользуй уже полученное exact-SHA evidence. Не повторяй полный lint/build/typecheck на каждой итерации, если затронутые после green-прогона файлы не делают результат устаревшим; запускай минимальный affected gate и полный релевантный набор перед публикацией принятого SHA.
- OpenAPI/generated gates запускай только когда менялись API contracts, route scopes или generated consumers.
...
- Для redesign-срезов и дизайн-прототипов QA должен проверять не только diff/build, но и реальные URLs, desktop/mobile layout, light/dark theme, role-specific pages and basic interactions. Отсутствие screenshots не является handoff gap; обязательны live URL/account/checklist, а screenshots нужны только по явному запросу или для внутренней диагностики.
