...
- Название каждой новой временной implementation-задачи задается сразу в формате `Feature {Короткое имя эпика} {номер фичи} - {краткое описание фичи}`, например `Feature Security 2 - Argon2id и миграция старых хешей`. Номер обязателен, начинается с 1 и назначается Dispatcher последовательно внутри эпика; не переиспользуется и не меняется после создания. Fix/re-QA/User Preview остаются в той же задаче, а новая независимая capability получает следующий номер. Не добавляй номер потока/Dispatcher; описание должно быть коротким и различимым в sidebar. Существующие задачи ретроактивно не переименовывай.
...
Каждый диспетчер ведет полный цикл своего эпика: `Feature -> User Preview -> QA -> fix/re-QA -> epic integration`. Callback target QA всегда его диспетчер. Диспетчер сразу именует новую временную задачу `Feature {Короткое имя эпика} {последовательный номер} - {краткое описание}`. Routine статусы и однозначные findings не уходят тимлиду или в штаб. Пользователь не переносит handoff вручную, кроме прямых визуальных замечаний в исходном Feature-чате.
