...
Статус: `baseline for implementation planning`
...
| Model / table | Scope | Текущий owner и связи | Целевой tenant key / можно ли вывести | Tenant-aware unique/index | Isolation risk | Backfill |
| --- | --- | --- | --- | --- | --- | --- |
| `AuditLog` / `AuditLogs` | D | Generic entity/action payload + Account; entity polymorphic | Прямой immutable `organizationId`, nullable `clubId`, `membershipId`; нельзя полагаться на удаляемый polymorphic parent | tenant + createdAt; tenant + entityType/entityId; tenant + action/account | **P1:** audit export/search смешивает tenants, удаленный entity теряет owner | 6: derive entity tenant by type; exception ledger for unknowns |
...
| Cron/background | `server/bot.js` starts global recurring call-task runner and Beeline subscription runner; global advisory lock | Per-tenant cursor/config; lock name includes connection/club; job payload immutable tenant; failure/dead-letter metrics partitioned | **P1** wrong-club processing and head-of-line blocking |
