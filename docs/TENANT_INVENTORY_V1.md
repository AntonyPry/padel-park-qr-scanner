...
## 14. Feature 9 enforcement closure

- Definition/data-aware final audit расширяет принятые migration/domain classifiers и проверяет exact columns, tenant-leading indexes, composite FKs, triggers, immutability и same-parent invariants. Historical migrations не переписываются; исправления schema выполняются только additive forward migration.
- Broad detector находится в `server/src/tenant-enforcement/integrity-detector.js`, CLI — `server/scripts/tenant-integrity-detector.js`. Report read-only и fail-closed; zero findings обязателен для RC fixture, а seeded representative violations должны быть обнаружены.
- Exact singleton bridge централизован в `server/src/tenant-enforcement/legacy-singleton.js`. Он применяется к HTTP, service, provider ingress, worker/file resolution, realtime publishing и sockets; второй tenant несовместим с legacy business path.
- Final direct-write/alias/route gate находится в `server/scripts/audit-final-tenant-enforcement.js` и запускает все существующие domain audits отдельно, сохраняя их собственные allowlists и failure semantics.
- Test-only builder `server/tests/helpers/final-tenant-rc-fixture.js` создаёт две Organizations × два Clubs × шесть ролей и одинаковые natural identities. Production seeders, provisioning paths и Feature 10 UI не добавлены.
- Полная installation restore rehearsal находится в `server/tests/tenant-enforcement/installation-backup-restore.db.test.js`; tenant-selective manifest, incomplete artifact set, checksum mismatch и unsafe worker policy отклоняются, а restore target создаётся как fresh disposable installation.
- Принятые persisted/API поля не расширялись без consumer: Feature 9 использует существующие tenant/session/provider/storage attribution; новые public response fields отсутствуют.

## 15. Completeness evidence
