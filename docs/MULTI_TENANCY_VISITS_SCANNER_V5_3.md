...
5. Добавляет composite FKs Visit→Club/User, ScannerEvent→Club/Visit/User и Assignment→Visit/VisitCategory. Для `duplicateOfVisitId` сохраняет legacy FK `ON DELETE SET NULL`, а DB insert/update triggers запрещают cross-Organization/cross-Club self-link без изменения прежней delete semantics. Cross-Organization client/category также физически невозможны.
...
Migration распознаёт `legacy`, `partial` и полностью `ready` schema по columns, всем named constraints/indexes/triggers. Forced failure после backfill удаляет partial DDL, восстанавливает legacy constraints/indexes и оставляет business rows неизменными. DB regression подтверждает production-shaped forced failure, `down → up → down → up`, неизменные counts/checksum и rollback refusal при втором Organization. Schema down — только development/emergency path: exact single-default, valid graph и отсутствие later migrations.
...
Visits sheet в XLSX, source-quality export, segment preview/count/list и client-base snapshot используют тот же context. Finance/receipt/booking aggregates внутри revenue coverage не объявляются tenant-safe этим срезом: их полная isolation остаётся отдельной wave; Visit cohort и canonical client population уже tenant-scoped, а production по-прежнему hard-gated в single-default mode.
