...
- For Shift Cash releases, verify onboarding covers `/admin/shift-cash` for opening cash, cash expense with receipt photo, closing reconciliation and variance comment; verify related shift report lessons open `/admin/shift-reports`; verify linked `Finance` expense in P&L/export and training cleanup for `ShiftCashSession`, `ShiftCashExpense`, attachments and linked finance rows.
...
- Verify Shift Cash on `/admin/shift-cash`: admin records opening cash and expense with photo, owner/manager close cash with required variance comment, `/admin/shift-reports` opens separately, accountant sees the linked expense in `/admin/finances`, and onboarding checkpoints are recorded by backend events `shift_cash.opening_recorded`, `shift_cash.attachment_uploaded` and `shift_cash.closed`.
