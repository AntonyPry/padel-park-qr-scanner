...
- Verify Shift Cash on `/admin/shift-cash`: admin records opening cash and expense with photo, owner/manager close cash with required variance comment, related shift reports open on `/admin/shift-reports`, accountant sees the linked expense in `/admin/finances`, and onboarding checkpoints are recorded by backend events `shift_cash.opening_recorded`, `shift_cash.attachment_uploaded` and `shift_cash.closed`.
