...
- For Shift Cash releases, verify onboarding covers `/admin/shift/cash` inside the `Смена` workspace for opening cash, cash expense with receipt photo, closing reconciliation and variance comment; verify shift report lessons open `/admin/shift/reports`; verify motivation rule/settings lessons open `/admin/shift-settings`; verify linked `Finance` expense in P&L/export and training cleanup for `ShiftCashSession`, `ShiftCashExpense`, attachments and linked finance rows.
- For this Shift Cash schema recreation release, the current production cash session is disposable test data. Create and verify a database backup, stop the backend, run `npx sequelize-cli db:migrate:undo --env production --name 20260714100000-create-shift-cash.js`, then run `npx sequelize-cli db:migrate --env production`. Confirm that `ShiftCashExpenses` has no `categoryId` or `categoryName`, restart the backend, and verify health, API smoke and browser smoke before switching traffic.
- For Shift Report Templates releases, verify owner/manager onboarding covers `/admin/shift-settings`: template cards list, create modal, active edit modal with save/archive, archived read-only modal with restore, and no obsolete template scope fields in text or screenshots.
...
- Verify Shift Cash on `/admin/shift/cash`: admin records opening cash and expense with photo, owner/manager close cash with required variance comment, related shift reports open on `/admin/shift/reports`, shift settings open on `/admin/shift-settings`, accountant sees the linked expense in `/admin/finances`, and onboarding checkpoints are recorded by backend events `shift_cash.opening_recorded`, `shift_cash.attachment_uploaded` and `shift_cash.closed`.
- Verify Shift Report Templates on `/admin/shift-settings`: owner/manager can open cards by click/Enter/Space, create a template, edit/save an active template, archive it, open archived read-only state, restore it from the modal, and see only current template fields.
