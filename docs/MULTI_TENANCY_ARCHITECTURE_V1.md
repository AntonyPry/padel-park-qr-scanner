...
Единственный supported production write path — централизованный transactional account lifecycle service. Существующие `accounts.service`, bootstrap/default-owner path в `auth.service`, demo fixture helpers и будущие Account mutations обязаны вызывать его; прямые production `Account.create/update/destroy` вне bridge запрещены. Bulk operations либо проходят по тому же bridge в одной управляемой batch transaction, либо явно отклоняются. Startup parity assertion ничего не чинит молча: divergence останавливает startup/release.

Lock order для каждой mutation постоянный: default Organization → target Account/Membership → MembershipClubAccess rows. Для owner-affecting mutation после lock Organization загружаются active owner Memberships `FOR UPDATE`; guard и все последующие writes выполняются в той же Sequelize transaction.
...
| --- | --- | --- | --- |
| Feature 2 — tenant foundation + lifecycle bridge | Feature 1 QA | Additive models/migrations `Organization`, `Club`, `Membership`, `MembershipClubAccess`; DB-enforced composite same-org FKs; exact default backfill; transactional Account create/update/archive/restore/permanent-delete parity bridge; no Membership.staffId yet; DB-backed lifecycle/concurrency tests; startup/migration/release assertion. Account-based reads/auth remain unchanged. | `down` только после exact six-step preflight; runtime reads/auth продолжают старую single-club Account model. |
...
- Feature 2 обязана добавить один общий assertion, используемый post-migration check, application startup, release gate и rollback preflight: `Organizations.count = 1`, `Clubs.count = 1`, exact default slugs/status/relationship совпадают; каждый Account имеет parity Membership; owner не имеет access, non-owner имеет один parity access; orphan/extraneous rows отсутствуют; last active owner существует.
