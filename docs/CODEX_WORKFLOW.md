...
Название новой временной implementation-задачи сразу задается в формате:

- `Feature {Короткое имя эпика} {номер фичи} - {краткое описание фичи}`.

Номер обязателен, начинается с 1 и назначается Dispatcher последовательно внутри
эпика. Он не переиспользуется и не меняется после создания. Fix/re-QA/User Preview
остаются в той же задаче; новая независимая capability получает следующий номер.
Номер потока/Dispatcher в title не добавляется. Описание должно быть коротким и
различимым в sidebar. Существующие задачи ретроактивно не переименовываются.
...
Каждый чат отражает текущее операционное состояние одним ведущим marker:

- `🟢 <базовое название>` — прямо сейчас выполняет полезную работу, включая
  выполняемую или наблюдаемую команду/тест;
- `❗️ <базовое название>` — реально остановлен до конкретного ответа, решения,
  доступа, действия или User Preview acceptance пользователя;
- `🔄 <базовое название>` — реально остановлен до результата другого чата/роли
  и не имеет полезной независимой работы;
- без marker — idle/unassigned, completed или paused без активного ожидания.

Markers взаимоисключающие и не накапливаются. При переходе чат сам
заменяет/снимает только ведущий marker, сохраняя актуальное dynamic name.
Неблокирующий вопрос не снимает `🟢`; параллельная dependency не требует `🔄`,
пока чат может полезно продолжать. Dispatcher не меняет marker дочерней задачи
за неё; Feature сохраняет прямой User Preview dialogue.
