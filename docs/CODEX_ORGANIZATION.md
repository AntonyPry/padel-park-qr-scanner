...
- сразу именует временную задачу `Feature <Короткое имя эпика> — <Конкретная capability>`;
...
Безликие названия временных задач вроде `Feature 1`, `Исправления` или одного
имени эпика не используются.
...
`!!!`. Квадратные скобки не используются, второй marker не добавляется.
