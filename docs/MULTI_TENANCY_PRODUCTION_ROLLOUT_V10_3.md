...
Единственное узкое исключение — exact canonical Beeline capability route во
время отдельно описанного provider URI cutover. Оно требует одновременно
`SETLY_BEELINE_CAPABILITY_CUTOVER_ENABLED=true`, full-stop и успешную
pre-body capability authentication. Bare/header routes, другие API и
неверный capability исключения не получают.
...
- разрешены только `/installation`, `/installation/provisioning`, статические
  frontend assets под `/assets/`, точные brand files
  `/brand/setly-mark-black.png`, `/brand/setly-mark-white.png` и root files `/favicon.ico`,
...
Preflight также read-only считает существующие Beeline/Evotor roots до
migrations. Непустые historical roots при отсутствующей или частичной provider
configuration блокируют maintenance заранее. Для Beeline сочетание API base,
API token и callback base без `BEELINE_WEBHOOK_SECRET` является полноценным
`capability_uri` mode; токен callback создаётся только внутри bootstrap
transaction и никогда не печатается. Имена providers и counts выводить можно,
credentials, ciphertext и callback capability — нельзя.
...
`SETLY_BEELINE_CAPABILITY_CUTOVER_ENABLED=false` и все 16 tenant flags в
`false`. На live preflight 19 июля 2026 года имена
...
npm run tenant:providers:beeline:cutover -- --dry-run
...
### 4. Maintenance-only canonical Beeline callback cutover

Это обязательный controlled-downtime переход. Нулевой downtime не заявляется:
исторический bare callback не аутентифицировался, а реальные XML event classes
не имеют 100% authoritative subscription match. Новый release поэтому никогда
не открывается с bare route; `subscriptionId` не используется как замена
аутентификации.

Bootstrap в одной transaction создаёт или точно переиспользует default Beeline
connection, сохраняет random 256-bit capability только в encrypted secret bundle
и связывает все historical calls/raw events/subscriptions. На принятом
production-shaped baseline это `1,467 + 8,607 + 1,509 = 11,583` roots.
Повторный bootstrap обязан сохранить `publicId`, ciphertext/key version, config,
...
1. не выполнять `tenant:providers:beeline:cutover -- --apply`;
2. оставить старый provider callback URI без изменений;
3. остановить новый process, вернуть old application/env с tenant flags off;
4. не down-migrate и не удалять additive columns, connection или reconciled
   attribution;
5. подтвердить old health/legacy callback и только затем открыть старый traffic.

После green post-migration gate и до URI switch выполнить под full-stop:

```bash
cd /opt/padel-park-qr-scanner/server
npm run tenant:providers:beeline:cutover -- --dry-run

# The callback capability remains encrypted and is never exported to shell.
# Enable only while global full-stop remains active.
export SETLY_BEELINE_CAPABILITY_CUTOVER_ENABLED=true
pm2 restart "$SETLY_PM2_APP" --update-env

npm run tenant:providers:beeline:cutover -- --apply
```

`--apply` делает provider PUT напрямую на transient canonical capability URI.
Команда строит URI только в памяти и печатает sanitized result. В этот момент
старый callback становится недействующим; callbacks, пришедшие в accepted
maintenance interval, могут быть недоступны. Bare route не получает maintenance
exception и всегда отвечает `404` до parser/write.

Сразу после PUT выполнить один controlled business call и доказать counts-only:
новый raw event и call пришли через exact capability route, имеют default
organization/club и exact Beeline `integrationConnectionId`; overlap/retry не
создал duplicate. Не выводить payload, phone, subscription ID или callback URI.
Затем, не снимая full-stop, включить dependency prefix через
`TENANT_PROVIDER_INTEGRATIONS_ENABLED=true`, restart и повторить controlled
capability ingress smoke. Route обязана работать и при flag off, и при flag on.

Только после этих доказательств одним restart убрать одновременно maintenance и
исключение: `SETLY_BEELINE_CAPABILITY_CUTOVER_ENABLED=false`,
`SETLY_ROLLOUT_MAINTENANCE_MODE` unset, approved prefix flags остаются. Bots,
worker и background runners пока не запускать. Открывать traffic с exception
`true` запрещено.

После URI switch rollback к old release или unauthenticated callback запрещён.
Recovery выполняется только на новом release под full-stop:

1. оставить capability connection/key и canonical route рабочими;
2. снова включить узкое exception и restart backend;
3. повторить dry-run, затем provider PUT `--apply`;
4. повторить controlled call и attribution/idempotency checks;
5. убрать exception вместе с full-stop и лишь затем открыть traffic.

Если provider PUT нельзя подтвердить, production остаётся закрытым; ручное
редактирование provider rows, создание fake webhook secret, query secret и
возврат bare callback запрещены.

### 5. Staged flag cutover
